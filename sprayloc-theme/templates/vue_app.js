...
<div id="folders-bar">
    ...
    methods: {
      ...
      onMouseOver(item) {
        ...
        if (item) {
          ...
          let subfolder_div = document.getElementById(
            "subfolder-" + item.displayname
          );
  ...
  const DetailVue = Vue.component("detail-vue", {
    ...
    methods: {
      ...
      isAKit: function (item) {
        return (
          this.kits.find((element) => {
            let equipID = parseInt(
              element.equipment.replace("/equipment/", "")
            );
            let parentID = parseInt(
              element.parent_equipment.replace("/equipment/", "")
            );
      ...
      getParentEquipmentID: function (item) {
        ...
        let parentID = parseInt(
          found.parent_equipment.replace("/equipment/", "")
        );
  ...
  const Pagination = Vue.component("sprayloc-pagination", {
    ...
    methods: {
      ...
      onNextPage: function () {
        if (
          this.current_page < Math.ceil(this.filtered.length / this.maxitems)
        ) {
          this.$router.push({
            path: "/",
            query: {
  ...
  var app = new Vue({
    ...
    data: function () {
      return {
        ...
        placeholder_url:
          "wp-content/themes/sprayloc-theme/assets/sprayloc_logo_placeholder.jpg",
    ...
    methods: {
      ...
      showDetails: function (item_id) {
        ...
        setTimeout(() => {
          ...
          const nav_link_desc = document.querySelectorAll(
            ".nav-tabs .nav-item a"
          );
          nav_link_desc[0].classList.add("active", "show");
          if (nav_link_desc[1])
            nav_link_desc[1].classList.remove("active", "show");
      ...
      getData() {
        ...
        if (sessionStorage.sprayloc_data === undefined) {
          ...
          if (data_age > this.max_minutes_local_data * 60) {
            //10 minutes
      ...
      getImageFile: function (file_id) {
        if (this.files.length > 0) {
          ...
          if (filtered) return filtered.url;
          else return this.placeholder_url + "sdfsdf";
        }
      },
      ...
      getImageThumbnail: function (file_id) {
        ...
        if (this.files) {
          ...
          if (filtered !== undefined) {
            ...
            let link =
              "wp-content/themes/sprayloc-theme/inc/gallery/" +
              thumb_name +
              ext;
    ...
    computed: {
      filtered: function () {
        ...
        if (this.data_loaded) {
          ...
          let pattern =
            this.string_filter != undefined ? this.string_filter : "";
          ...
          if (pattern != "") {
            filtered_equipments = this.equipment.filter((value) => {
              ...
              for (let word of words) {
                let found_index = this.sanitizeName(value.name)
                  .toLowerCase()
                ...
                if (found_index != -1) {
                  ...
                  let found_index2 = this.sanitizeName(folder_name)
                    .toLowerCase()
                    .search(this.sanitizeName(word).toLowerCase());
      ...
      paginated_items: function () {
        if (this.filtered) {
          let paginated = this.getPaginatedItems(
          ...
          if (this.filtered.length < this.pagination_max)
            num = this.filtered.length;
          for (let i = 0; i < num; i++) {
